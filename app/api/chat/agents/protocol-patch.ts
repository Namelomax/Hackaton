...
ПРАВИЛА:
1. Каждый "find" — ТОЧНАЯ подстрока из текущего протокола, скопированная символ в символ (кавычки, пунктуация, регистр). Значок ⚠️ в "find" НЕ пиши: в тексте маркер выглядит как «требует уточнения» или «подлежит уточнению», значок добавляется только при показе.
