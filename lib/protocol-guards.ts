...
/**
 * ФИО из 1-5 слов с заглавной буквы: «Иванов И.И.», «Стрыгину Константину», «Савинкин Михаил Евгеньевич».
 * Верхняя граница пять, а не три: модель иногда сдваивает имя («Киреев Алексей Алексей Васильевич»).
 */
const NAME_RX = '[А-ЯЁ][а-яё]+(?:\\s+[А-ЯЁ](?:[а-яё]+|\\.)(?:\\s?[А-ЯЁ]\\.)?){0,4}';
/** «Заказчик (Иванов И.И.)» -> «Заказчик». Сторона в любом падеже. Внутри скобок допускается список ФИО через запятую: «Исполнитель (Быкова А.Н., Ступина М.)». */
const SIDE_WITH_NAME_RX = new RegExp(`(${SIDE_ANY_RX})\\s*\\(${NAME_RX}(?:\\s*,\\s*${NAME_RX})*\\)`, 'g');
...
/** Сторона строго в именительном падеже: «Заказчик», «Исполнитель» (не «Заказчику», не «Исполнителем»). */
const NOMINATIVE_SIDE_RX = /(Заказчик|Исполнитель)(?=[\s,.;:!?)]|$)/g;
...
/**
 * Позиции последних упоминаний «Заказчик»/«Исполнитель» строго в именительном
 * падеже (без падежных окончаний) в переданном тексте. -1, если не встречались.
 */
function lastNominativeSideIndexes(text: string): { customer: number; executor: number } {
  let customer = -1;
  let executor = -1;
  for (const m of text.matchAll(NOMINATIVE_SIDE_RX)) {
    if (m[1] === 'Заказчик') customer = m.index;
    else executor = m.index;
  }
 ...
 * упомянутую до этого места сторону.
 *
 * Антецедент ищем в два шага:
 * 1) сначала СТРОГО именительный падеж («Заказчик», «Исполнитель») — иначе
 *    «Исполнитель направит Заказчику отчёт. Он отметил…» подставил бы
 *    «Заказчик», хотя подлежащее предложения — «Исполнитель» (косвенный
 *    падеж «Заказчику» — дополнение, а не подлежащее);
 * 2) если именительной формы до этого места не было — фолбэк на любую форму
 *    («Заказчику», «Исполнителем» и т.п.), чтобы не терять антецедент вовсе.
 * Если до этого места ни одна сторона не встречалась ни в каком падеже —
 * местоимение не трогаем (лучше оставить как есть, чем подставить неверную сторону).
 */
function replaceLeadingPronouns(text: string): string {
  return text.replace(SENTENCE_START_PRONOUN_RX, (whole, lead, pronoun, tail, offset) => {
    ...
    let { customer: lastCustomer, executor: lastExecutor } = lastNominativeSideIndexes(before);
    if (lastCustomer < 0 && lastExecutor < 0) {
      lastCustomer = before.toLowerCase().lastIndexOf('заказчик');
      lastExecutor = before.toLowerCase().lastIndexOf('исполнител');
    }
 ...
 * «Ответственный: Заказчик (Иванова А.).». Промпт это просит, но модель
...
export function enforceSideNamingInText(text: string): string {
  ...
  // 1. Снимаем ФИО в скобках после стороны (в т.ч. в метке «Ответственный: …»).
  let next = text.replace(SIDE_WITH_NAME_RX, '$1');
  // 2. Согласуем род глагола (женский -> мужской) сразу после стороны.
  next = fixSideVerbGender(next);
  // 3. Заменяем местоимение в начале предложения на последнюю упомянутую сторону.
  next = replaceLeadingPronouns(next);
  // 4. После подстановки стороны вместо «Она» глагол мог остаться в женском роде.
  next = fixSideVerbGender(next);
