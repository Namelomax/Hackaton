...
/**
 * Отправляет запрос к шлюзу и, если тот успел подменить выложенную модель
 * (см. историю отказов у FIXED_CHAT_MODEL в lib/chat-models.ts), делает ОДИН
 * повторный запрос с актуальным именем — без правки env и без рестарта.
 *
 * Общая точка для ОБОИХ путей отправки — стримингового и не-стримингового
 * (см. LLM_FORCE_NONSTREAM ниже), чтобы 404-детект не разъезжался копипастой.
 *
 * Тело ответа читаем ТОЛЬКО когда status === 404 — на остальных статусах,
 * включая стриминговый путь, поток уходит вызывающему нетронутым.
 */
async function fetchWithModelRetry(
  url: string | URL | globalThis.Request,
  init: RequestInit,
  parsed: Record<string, unknown>,
): Promise<Response> {
  const resp = await insecureFetch(url, init);
  if (resp.status !== 404) return resp;

  const text = await resp.text();
  if (!text.includes('does not exist')) {
    // Другой повод для 404 (не смена модели) — тело уже прочитано, пересобираем Response как было.
    return new Response(text, { status: resp.status, headers: resp.headers });
  }

  const oldModel = parsed.model;
  const fresh = await discoverGatewayModel(true);
  if (!fresh) {
    // Перепроверка не удалась (шлюз недоступен и т.п.) — ретраить нечем, отдаём исходный 404.
    return new Response(text, { status: resp.status, headers: resp.headers });
  }

  parsed.model = fresh.id;
  console.log(`🔄 [llm] модель на шлюзе сменилась: ${String(oldModel)} → ${fresh.id}, повтор запроса`);
