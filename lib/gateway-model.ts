 ...
 * обнаружение вызывается перед каждым запросом к шлюзу из resolve-chat-model.ts;
 * кэш с TTL делает это дешёвым. Форсированный повтор (force=true) остаётся
 * только на 404-ретрае при переименовании модели.
 *
