...
describe("assembleDocumentXml", () => {
  ...
  it("падает понятной ошибкой на шаблоне без <w:document>", () => {
    expect(() =>
      assembleDocumentXml(
        '<w:body><w:sectPr><w:pgSz w:w="11906"/></w:sectPr></w:body>',
        "",
      ),
    ).toThrow(/w:document/);
  });
  ...
  it("берёт финальный sectPr, а не вложенный в разрыв раздела", () => {
    ...
    const xml = assembleDocumentXml(
      templateWithSectionBreak,
      "<w:p>новое</w:p>",
    );
...
describe("renderProtocolDocx", () => {
  ...
  it("переносит оформление из шаблона байт-в-байт", async () => {
    ...
    const result = await JSZip.loadAsync(
      await renderProtocolDocx(SAMPLE_PROTOCOL),
    );
    ...
    for (const part of parts) {
      ...
      // Часть заведомо есть в обоих zip (список part — из самого шаблона),
      // поэтому undefined тут означал бы падение теста раньше, на .equals.
  ...
  it("не тащит шрифты в результат", async () => {
    ...
    expect(
      Object.keys(zip.files).filter((n) => n.startsWith("word/fonts/")),
    ).toEqual([]);
  });
