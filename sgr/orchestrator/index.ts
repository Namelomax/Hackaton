...
ВАЖНО: Протокол обследования должен содержать ВСЕ 10 разделов:
1. Номер протокола и дата встречи
2. Повестка встречи
3. Участники (со стороны Заказчика и Исполнителя с полными ФИО и должностями)
4. Термины и определения
5. Сокращения и обозначения
6. Содержание встречи
7. Вопросы и ответы
8. Решения (с обязательным указанием ответственных)
9. Открытые вопросы
10. Согласовано (подписанты с обеих сторон)

КРИТИЧЕСКИ ВАЖНО:
- Документ ГОТОВ только если ВСЕ 10 разделов заполнены и подтверждены пользователем
- Если заполнено менее 10 разделов — продолжай сбор информации
- Раздел "Участники" должен содержать полные ФИО (Фамилия Имя Отчество) и должности
- Раздел "Решения" должен содержать ответственных для каждого решения
- НЕ предлагай генерацию документа после сбора 1-2 разделов!
...
ВАЖНО: Различай эти намерения:

**DOCUMENT (запрос документа):**
- Пользователь явно просит сформировать/показать протокол
- Пользователь подтверждает готовность ("да", "готово", "показывай")
- Пользователь отвечает на вопрос ассистента о готовности документа

**CHAT (продолжение диалога/сбора информации):**
- Пользователь отвечает на уточняющие вопросы ассистента
- Пользователь просит перейти к следующему разделу/этапу/вопросу
- Пользователь использует команды: "переходи", "продолжай", "дальше", "следующий"
- Пользователь уточняет информацию или даёт новые данные
- Это любая другая ситуация, когда документ ещё не готов к формированию

Критически важно: команды типа "переходи к следующему разделу", "продолжай", "дальше" — это CHAT (продолжение сбора информации), а НЕ запрос документа!

Что хочет пользователь: продолжить чат (сбор информации) или получить документ?
`,
      temperature: 0.1,
    });
